...
mod tests {
    ...
    use crate::graph::{
        graph::Graph,
        types::{GraphEdge, GraphGroup, GraphIIP, GraphJson, GraphNode},
    };
                ...
                'and_when_given_a_new_node: {
                    'then_it_should_emit_an_event: {
                        g.connect(
                            "add_node",
                            |this, data| {
                                let node = data.downcast_ref::<GraphNode>();
                ...
                'and_when_given_a_new_edge: {
                    'then_it_should_emit_an_event: {
                        ...
                        g.connect(
                            "add_edge",
                            |this, data| {
                                if let Some(edge) = data.downcast_ref::<GraphEdge>() {
            ...
            'when_given_a_json_string: {
                ...
                'then_it_should_parse: {
                    ...
                    'and_then_it_should_produce_a_graph: {
                        let g = block_on(Graph::from_json_string(json_string, None));
                        assert_eq!(g.is_err(), false);
                    }
                    'and_then_it_should_produce_a_graph_from_json_object: {
                        let mut g = block_on(Graph::from_json(json.clone(), None));
                        ...
                        'and_then_it_should_allow_modifying_graph_metadata: {
                            g.connect(
                                "change_properties",
                                move |this, data| {
                                    if let Some(payload) = data.downcast_ref::<(
                                        HashMap<String, Value>,
                                        HashMap<String, Value>,
                                    )>(
                                    ) {
                                        let new_metadata = payload.clone().0;
                                        let old_metadata = payload.clone().1;
                            ...
                            g.connect(
                                ...
                                move |this, data| {
                                    if let Some(payload) = data.downcast_ref::<(
                                        ...
                                        let node = payload.clone().0;
                                        // let old_metadata = payload.clone().1;
                                        // let new_metadata = payload.clone().2;
                        ...
                        'and_then_it_should_allow_renaming_groups: {
                            if let Some(_) = g.groups.clone().get(0) {
                                g.connect(
                                    ...
                                    move |this, data| {
                                        if let Some(payload) =
                                            data.downcast_ref::<(String, String)>()
                                        {
                                            let old_name = payload.clone().0;
                                            let new_name = payload.clone().1;
                                ...
                                'and_then_it_should_allow_removing_a_node: {
                                    'and_then_it_should_emit_an_event: {
                                        g.connect(
                                            "remove_node",
                                            |this, data| {
                                                if let Some(node) = data.downcast_ref::<GraphNode>()
                                                {
                ...
                'then_when_removing_that_iip: {
                    'and_then_it_should_emit_an_event: {
                        g.connect(
                            "remove_initial",
                            |this, data| {
                                if let Some(iip) = data.downcast_ref::<GraphIIP>() {
            ...
            'when_saving_and_loading_files: {
                'then_when_saving_with_json_file_extension: {
                    ...
                    'and_then_it_should_be_able_save_file: {
                        ...
                        assert_eq!(block_on(g.save(graph_path)).is_err(), false);
                        assert_eq!(std::fs::File::open(graph_path).is_err(), false);

                        'and_then_it_should_be_possible_to_load_a_graph_from_file: {
                            if let Ok(result) = block_on(Graph::load_file(graph_path, None)) {
                                let original_graph = block_on(g.to_json());
                                assert_json_eq!(json!(block_on(result.to_json())), json!(original_graph));
                            } else {
        ...
        'given_without_case_sensitivity:{
            'then_graph_operations_should_convert_port_names_to_lowercase:{
                ...
                'and_then_it_should_have_case_insensitive_ports_on_the_edges:{
                    g
                    .add_node("Foo", "foo", None)
                    .add_node("Bar", "bar", None)
                    .connect("add_edge", |this, data| {
                        if let Some(edge) = data.downcast_ref::<GraphEdge>() {
                            assert_eq!(edge.from.node_id, "Foo");
                    ...
                    g.add_edge("Foo", "outPut", "Bar", "inPut", None);
                }
            }
        }
    }
